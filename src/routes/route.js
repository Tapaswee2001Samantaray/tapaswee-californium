...
let persons= [
 ...
 //assignment on query params --> you will be given an array of persons ( i.e an array of objects )..each person will have  a {name: String , age: Number, votingStatus: true/false(Boolean)}
...
// also return an array consisting of only the person that can vote
 ...
 router.post("/age" , function(req , res){
    ...
    for(let i=0 ; i<persons.length ; i++){
        if(persons[i].age >= personObj.votingAge){
            persons[i].votingStatus = true;
            newArr.push(persons[i]);
        }
    }
    
    res.send({data:newArr , status:true});
 });
